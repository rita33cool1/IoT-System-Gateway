...
func (ls *layerStore) deleteLayer(layer *roLayer, metadata *Metadata) error {
    //--!YC--
    logrus.Info("Deleting layer: start")
    logrus.Info(string(layer.cacheID))
    logrus.Info(string(layer.chainID))
    //--!YC--
    ...
    //--!YC--
    logrus.Info("Deleting layer: end")
    //--!YC--
...
func (ls *layerStore) releaseLayer(l *roLayer) ([]Metadata, error) {
    ...
    //--!YC--
    file, err :=  os.Open("/home/minion/YC/YC-docker-engine/delete_layer_num")
    if err != nil {
       return nil, err
    }
    defer file.Close()
    ...
    dat, err:=  Readln(reader)
    num_delete_layer := dat
    cnt := 0
    //--!YC--
	...
	for {
        ...
        //--!YC--
        // delete until delete the specific num of layer
        cnt++
        logrus.Info("[Num delete layer]"+num_delete_layer+strconv.Itoa(cnt))
        if strconv.Itoa(cnt) == num_delete_layer {
            logrus.Info("Equal to algorithm: return")
		    return removed, nil
        }
        //--!YC--
